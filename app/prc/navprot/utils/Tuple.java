...
public class Tuple implements Serializable {
     ...
     * ( max(|this(i) - t(i)|) < epsilon ).
     * Tuple sizes must match and the epsilon value must be nonnegative. 
     * @param t The tuple we compare.
     * @param epsilon The L infinity norm of the difference 
     *                between tuples is compared to this value.
     */
     ...
     *                between tuples is compared to this value.
     */
    public boolean equalsCartesian(Tuple t, double epsilon) {
	if(t.data.length != data.length)
	    throw new IllegalArgumentException("Tuples must have same "+
					       "dimenssion");
	if(epsilon < 0)
	...
	for(int i=0; 3*i<data.length; i++) {
            double distance = Math.pow( (data[3*i]-t.data[3*i])*(data[3*i]-t.data[3*i])
                             + (data[3*i+1]-t.data[3*i+1])*(data[3*i+1]-t.data[3*i+1])
                             + (data[3*i+2]-t.data[3*i+2])*(data[3*i+2]-t.data[3*i+2]),0.5);
     ...
     *                between tuples is compared to this value.
     */
    public boolean equalsSpecial(Tuple t, double epsilon) {
	if(t.data.length != data.length)
	    throw new IllegalArgumentException("Tuples must have same "+
					       "dimenssion");
	if(epsilon < 0)
	...
	for(int i=3; i<data.length; i++) {
            if(i!=4){
//                double err = data[i] - t.data[i];
                double val=(Math.abs( ((int)(100000*(Math.abs(data[i]-t.data[i]))) % (int)(100000*2*Math.PI)) / 100000.0 ));
                if(Math.abs(val) > epsilon)
                    return false;
            }
	}
